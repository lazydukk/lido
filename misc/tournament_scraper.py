...
class CausewayScraperDriver:
    ...
    """using hardcoded values for now; todo: prompt a userinput?"""
                    ...
                    pairings.append(
                        {
                            ...
                            "player_2_score": cells[4].text.strip()
                            if len(cells) > 4
                            else None,
                        }
                    )
    ...
    def scrape_player_profile(self, player_id: int) -> Optional[Dict]:
        ...
        stats_section = soup.find("div", {"class": "stats"}) or soup.find(
            "div", {"class": "profile"}
        )
        if stats_section:
                    ...
                    matches.append(
                        {
                            ...
                            "opponent_score": cells[3].text.strip()
                            if len(cells) > 3
                            else None,
                            "result": "W"
                            if int(cells[2].text.strip().split()[0])
                            > int(
                                cells[3].text.strip().split()[0]
                                if len(cells) > 3
                                else 0
                            )
                            else "L",
